...
VulkanContext::~VulkanContext()
{
	...
	std::cout << "VulkanContext destroyed successfully." << std::endl;
}
...
void VulkanContext::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags memoryflags, VkImage& image, VkDeviceMemory& memory)
{
...
SwapChainSupportDetails VulkanContext::QuerySwapChainSupport(VkPhysicalDevice device)
{
	...
	if (formatCount != 0) {
		...
		std::cout << "Device has no supported surface formats." << std::endl;
	}
